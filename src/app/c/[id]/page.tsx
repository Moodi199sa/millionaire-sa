...
// صفحة هبوط لمشاركة الكرت الحقيقي: id هو رقم الأشهر، وصورة الـ OG تشير لنفس
// PNG الذي ولّده متصفح المستخدم (مخزّن عبر /api/share-upload وتُعرض عبر
// /api/card-image/[id]) — لا إعادة بناء للتصميم على السيرفر إطلاقاً.
export async function generateMetadata(
  { params }: { params: { id: string } }
): Promise<Metadata> {
  const m = parseInt(params.id, 10)
  ...
  const ogImage = `/api/card-image/${params.id}`
...
export default function CardSharePage({ params }: { params: { id: string } }) {
  const m = parseInt(params.id, 10)
  const valid = Number.isFinite(m) && m > 0
